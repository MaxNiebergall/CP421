...
for i in range(len(item_pairs_list)):
	for j in range(i):
		if(i != j):
			first = item_pairs_list[i][0].split(" ")
			second = item_pairs_list[j][0].split(" ")
			if first[0] == second[0]:
...
item_trips_list = list(item_trips_dict.items())
item_trips_list.sort()
...
for k,v in list(item_trips_dict.items()):
    items_local = k.split(" ")
    conf = item_pairs_dict.get(items_local[0] + " " + items_local[1])
    item_pairs_list_sorted_by_conf.append((v/conf, k))
    
item_pairs_list_sorted_by_conf.sort()    
    
for k,v in item_pairs_list_sorted_by_conf:
    trips_fv.write(str(k) + " " + str(v) + "\n")
